...
def init_trainable_parameters(model):
    ...
    for name, module in model.named_modules():
        ...
        if hasattr(module, 'deq_routing_adapter_T_w2'):
            module.deq_routing_adapter_T_w2.weight.data.zero_()
            module.deq_routing_adapter_T_w2.weight.requires_grad = True
            if hasattr(module, 'deq_routing_adapter_G_w2'):
                module.deq_routing_adapter_G_w2.weight.data.zero_()
                module.deq_routing_adapter_G_w2.weight.requires_grad = True
                module.deq_routing_adapter_gamma.data.fill_(1.0)
                module.deq_routing_adapter_gamma.requires_grad = True
            if hasattr(module, 'deq_routing_adapter_alpha'):
                import os as _os
                _a0 = float(_os.environ.get('DEQ_ALPHA_INIT', -0.5))
                module.deq_routing_adapter_alpha.data = \
                    module.deq_routing_adapter_alpha.data.float().fill_(_a0)
                module.deq_routing_adapter_alpha.requires_grad = True
            print(f"Identity-initialized DEQ routing adapter for {name}")
